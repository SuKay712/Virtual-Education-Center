...
export class InitialSchema1746267083631 implements MigrationInterface {
    name = 'InitialSchema1746267083631'
    ...
    public async up(queryRunner: QueryRunner): Promise<void> {
        ...
        await queryRunner.query(`CREATE TABLE \`class\` (\`id\` int NOT NULL AUTO_INCREMENT, \`name\` varchar(255) NOT NULL, \`time_start\` datetime NOT NULL, \`time_end\` datetime NOT NULL, \`content\` varchar(255) NULL, \`rating\` int NULL, \`comment\` varchar(255) NULL, \`price\` varchar(255) NULL, \`created_at\` timestamp NOT NULL DEFAULT CURRENT_TIMESTAMP, \`updated_at\` timestamp NOT NULL DEFAULT CURRENT_TIMESTAMP ON UPDATE CURRENT_TIMESTAMP, \`lectureId\` int NULL, \`meetingId\` int NULL, PRIMARY KEY (\`id\`)) ENGINE=InnoDB`);
        ...
        await queryRunner.query(`CREATE TABLE \`lecture\` (\`id\` int NOT NULL AUTO_INCREMENT, \`created_at\` timestamp NOT NULL DEFAULT CURRENT_TIMESTAMP, \`updated_at\` timestamp NOT NULL DEFAULT CURRENT_TIMESTAMP ON UPDATE CURRENT_TIMESTAMP, \`courseId\` int NULL, \`studentId\` int NULL, PRIMARY KEY (\`id\`)) ENGINE=InnoDB`);
        ...
        await queryRunner.query(`CREATE TABLE \`account\` (\`id\` int NOT NULL AUTO_INCREMENT, \`email\` varchar(255) NOT NULL, \`password\` varchar(255) NOT NULL, \`name\` varchar(255) NULL, \`gender\` enum ('Male', 'Female', 'Other') NULL, \`birthday\` varchar(255) NULL, \`phone\` varchar(255) NULL, \`avatar\` varchar(255) NULL, \`address\` varchar(255) NULL, \`isActived\` tinyint NOT NULL DEFAULT 0, \`role\` enum ('Student', 'Teacher', 'Staff', 'Admin') NOT NULL DEFAULT 'Student', \`created_at\` timestamp NOT NULL DEFAULT CURRENT_TIMESTAMP, \`updated_at\` timestamp NOT NULL DEFAULT CURRENT_TIMESTAMP ON UPDATE CURRENT_TIMESTAMP, UNIQUE INDEX \`IDX_4c8f96ccf523e9a3faefd5bdd4\` (\`email\`), PRIMARY KEY (\`id\`)) ENGINE=InnoDB`);
        ...
        await queryRunner.query(`ALTER TABLE \`lecture\` ADD CONSTRAINT \`FK_1ad668443f9317e68056f4a66b7\` FOREIGN KEY (\`studentId\`) REFERENCES \`account\`(\`id\`) ON DELETE NO ACTION ON UPDATE NO ACTION`);
    ...
    public async down(queryRunner: QueryRunner): Promise<void> {
        ...
        await queryRunner.query(`ALTER TABLE \`lecture\` DROP FOREIGN KEY \`FK_1ad668443f9317e68056f4a66b7\``);
